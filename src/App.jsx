...
const DARK={bg:"#0A0E17",bgCard:"#111827",bgSurface:"#1A1F2E",text:"#E5E7EB",textMuted:"#8A8F98",textDim:"#4B5563",border:"rgba(255,255,255,0.06)",accent:"#00E5A0",accentDark:"#00B87D",danger:"#FF6B6B",warning:"#F59E0B",info:"#3B82F6"};
const LIGHT={bg:"#F9FAFB",bgCard:"#FFFFFF",bgSurface:"#F3F4F6",text:"#111827",textMuted:"#6B7280",textDim:"#9CA3AF",border:"rgba(0,0,0,0.08)",accent:"#059669",accentDark:"#047857",danger:"#DC2626",warning:"#D97706",info:"#2563EB"};
...
function sim(port,months,hr,hc,rebal,initAmt){
  ...
  const rebalInterval=rebal==="monthly"?1:rebal==="quarterly"?3:12;
  months.forEach((month,idx)=>{
    let pr=0;en.forEach(x=>{
      ...
      if(idx%rebalInterval!==0)r*=(1+(rand()*0.005));
...
export default function App(){
  ...
  const[cumUnit,setCumUnit]=useState("amount");
        ...
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",flexWrap:"wrap",gap:12,alignItems:"flex-end"}}>
          ...
          <div style={{flex:"1 1 auto"}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
              <select value={benchmark} onChange={e=>setBenchmark(e.target.value)} style={{fontSize:11,padding:"4px 8px",borderRadius:6,background:t.bgSurface,border:`1px solid ${t.accent}50`,color:t.accent,fontWeight:700,minWidth:80}}>
                {allP.filter(p=>sel.includes(p.id)).map(p=><option key={p.id} value={p.id}>★ {p.name}</option>)}
              </select>
          ...
          {/* 투자금액 + 통화 */}
          <div>
            <div style={{fontSize:9,color:t.textDim,marginBottom:4,fontWeight:700}}>투자금액</div>
            <div style={{display:"flex",gap:4,alignItems:"center"}}>
              <select value={currency} onChange={e=>setCurrency(e.target.value)} style={{fontSize:11,padding:"4px 6px",borderRadius:4,width:56}}>{CURRENCIES.map(c=><option key={c.id} value={c.id}>{c.sym} {c.id}</option>)}</select>
              <input type="number" value={initAmt} onChange={e=>setInitAmt(Math.max(1,+e.target.value))} style={{width:100,fontSize:11,textAlign:"right"}}/>
            </div>
          </div>
          <div style={{width:1,height:28,background:t.border,flexShrink:0}}/>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20,animation:"fadeIn .4s ease"}}>
          ...
          <div ref={secRef.cum} style={{background:t.bgCard,borderRadius:12,padding:16,border:`1px solid ${t.border}`,scrollMarginTop:140}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:3}}><Tab active={cumUnit==="amount"} onClick={()=>setCumUnit("amount")}>{curSym} 금액</Tab><Tab active={cumUnit==="pct"} onClick={()=>setCumUnit("pct")}>% 수익률</Tab></div>
            </div>
            <ResponsiveContainer width="100%" height={360}><LineChart data={chartData} margin={{top:5,right:5,left:10,bottom:0}}><CartesianGrid stroke={t.border} strokeDasharray="3 3"/><XAxis dataKey="date" tick={{fill:t.textDim,fontSize:10}} tickLine={false} axisLine={{stroke:t.border}} interval={Math.floor(chartData.length/8)}/><YAxis tick={{fill:t.textDim,fontSize:10}} tickLine={false} axisLine={false} tickFormatter={cumUnit==="amount"?v=>{if(v>=100000000)return`${(v/100000000).toFixed(1)}억`;if(v>=10000)return`${(v/10000).toFixed(0)}만`;return v.toLocaleString();}:v=>`${v}%`}/><Tooltip contentStyle={ttStyle} formatter={(v,n)=>{const p=allP.find(x=>cumUnit==="amount"?n===`${x.id}_v`:n===`${x.id}_p`);return[cumUnit==="amount"?fmtAmt(v):`${v}%`,p?.name||""];}}/>{activeP.map(p=><Line key={p.id} type="monotone" dataKey={cumUnit==="amount"?`${p.id}_v`:`${p.id}_p`} stroke={p.color} strokeWidth={1.8} dot={false}/>)}</LineChart></ResponsiveContainer>
          ...
          <div ref={secRef.yearly} style={{background:t.bgCard,borderRadius:12,padding:16,border:`1px solid ${t.border}`,scrollMarginTop:140}}>
            ...
            <div style={{fontSize:11,color:t.textMuted,marginBottom:12}}>기준: {bmPort?.name||"없음"} · 차트 바를 클릭하면 오른쪽에 상세 표시</div>
            {bmResult&&<div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
              <div style={{flex:"1 1 auto",minWidth:0}}>
                <ResponsiveContainer width="100%" height={360}>
                  <BarChart data={bmResult.yba} margin={{top:5,right:5,left:0,bottom:0}} onClick={e=>{if(e&&e.activeLabel)setSelYear(selYear===e.activeLabel?null:e.activeLabel);}}>
                    ...
                    <Tooltip contentStyle={ttStyle} formatter={(v,name)=>{const a=ga(name);return[`${Number(v).toFixed(1)}%`,a.name];}}/>
                    ...
                    {selYear&&<ReferenceLine x={selYear} stroke={t.accent} strokeWidth={2} strokeDasharray="none"/>}
                  </BarChart>
                </ResponsiveContainer>
              ...
              {selYear&&<div style={{width:280,flexShrink:0,background:t.bgSurface,borderRadius:10,padding:14,border:`1px solid ${t.accent}30`,animation:"fadeIn .3s",maxHeight:440,overflow:"auto"}}>
          ...
          <div ref={secRef.alloc} style={{background:t.bgCard,borderRadius:12,padding:16,border:`1px solid ${t.border}`,scrollMarginTop:140}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              {activeP.map(p=>{const r=results[p.id];if(!r)return null;return(
                <div key={p.id} style={{display:"flex",gap:20,alignItems:"center",padding:12,borderRadius:10,background:t.bgSurface,border:`1px solid ${t.border}`}}>
                  {/* 도넛 + 중앙 텍스트 */}
                  <div style={{position:"relative",width:180,height:180,flexShrink:0}}>
                    <ResponsiveContainer width={180} height={180}><PieChart><Pie data={r.en.map(a=>({name:a.name,value:a.w}))} cx="50%" cy="50%" innerRadius={50} outerRadius={75} dataKey="value" stroke={t.bgCard} strokeWidth={2}>{r.en.map((a,i)=><Cell key={i} fill={a.color}/>)}</Pie><Tooltip contentStyle={ttStyle} formatter={(v,name)=>[`${v}%`,name]}/></PieChart></ResponsiveContainer>
                    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",pointerEvents:"none"}}>
                      ...
                      <div style={{fontSize:14,fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:r.cagr>=0?t.accent:t.danger,marginTop:2}}>CAGR {r.cagr>0?"+":""}{r.cagr}%</div>
                    </div>
                  </div>
                  {/* 자산 목록 */}
                  <div style={{display:"flex",flexDirection:"column",gap:3,minWidth:160}}>
                    {r.en.map(a=><div key={a.a} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,color:t.textMuted,gap:12}}>
                      <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:6,height:6,borderRadius:"50%",background:a.color}}/>{a.name}</span>
                      <span style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:600,flexShrink:0}}>{a.w}%</span>
                    </div>)}
                  </div>
                  {/* 라인차트 */}
                  <div style={{flex:1,minWidth:200}}>
                    <ResponsiveContainer width="100%" height={140}><LineChart data={r.data} margin={{top:5,right:5,left:0,bottom:0}}><XAxis dataKey="date" tick={false} axisLine={false}/><YAxis tick={{fill:t.textDim,fontSize:9}} tickLine={false} axisLine={false} tickFormatter={v=>{if(v>=100000000)return`${(v/100000000).toFixed(1)}억`;if(v>=10000)return`${(v/10000).toFixed(0)}만`;return"";}}/><Tooltip contentStyle={ttStyle} formatter={v=>[fmtAmt(v),"자산"]}/><Line type="monotone" dataKey="value" stroke={p.color} strokeWidth={1.5} dot={false}/></LineChart></ResponsiveContainer>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:t.textDim,marginTop:2,padding:"0 4px"}}><span>{fmtAmt(initAmt)}</span><span>→ {fmtAmt(r.finalVal)}</span></div>
                  </div>
                </div>);})}
            </div>
          </div>
